...
class AwsCognitoIdentityGuard implements StatefulGuard
{
     ...
     * @var ?array
     */
    protected ?array $cognitoTokens = null;
     ...
     * @param  array  $config
     */
    public function __construct(string $name, CognitoIdentityProviderClient $client, UserProvider $provider, Session $session, Request $request, array $config = [])
    {
        $this->name = $name;
        $this->client = $client;
        ...
        if (isset($this->config['errors']['handler']) && $this->config['errors']['handler']) {
    ...
    public function user(): ?AuthenticatableContract
    {
        ...
        if (! is_null($id) && $user = $this->provider->retrieveById($id)) {
            if (! $this->getCognitoTokensFromSession()) {
                return null;
            }
        ...
        if (is_null($user) && ! is_null($recaller)) {
            ...
            if ($user) {
                if (! $this->getCognitoTokensFromRecaller($recaller)) {
                    return null;
                }
     ...
     */
    protected function getUserFromRecaller(Recaller $recaller): ?AuthenticatableContract
    {
     ...
     * @return null|array
     */
    protected function getCognitoTokensFromRecaller(Recaller $recaller): ?array
    {
     ...
     * @return null|array
     */
    protected function getCognitoTokensFromSession(): ?array
    {
     ...
     * @return null|array
     */
    protected function refreshCognitoTokens(string $refreshToken): ?array
    {
        try {
     ...
     * @return array
     */
    protected function addTokenExpiryTimes(array $tokens, bool $updateRefreshTokenExp = true): array
    {
     ...
     * @return AuthAttempt
     */
    protected function attemptCognitoAuthentication(array $credentials): AuthAttempt
    {
        if (
        ...
        try {
            ...
            return new AuthAttempt((bool) $response['AuthenticationResult'], $response->toArray());
        } catch (CognitoIdentityProviderException $e) {
     ...
     */
    protected function createRecaller(string $value): \Symfony\Component\HttpFoundation\Cookie
    {
     ...
     * @param  array  $credentials
     */
    protected function fireFailedEvent(?AuthenticatableContract $user, array $credentials)
    {
        if (isset($this->events)) {
     ...
     * @return string
     */
    public function getCognitoAccessToken(): string
    {
     ...
     * @return string
     */
    public function getCognitoIdToken(): string
    {
     ...
     * @return string
     */
    public function getCognitoRefreshToken(): string
    {
     ...
     * @return int
     */
    public function getCognitoTokensExpiryTime(): int
    {
     ...
     * @return int
     */
    public function getCognitoRefreshTokenExpiryTime(): int
    {
     ...
     * @return string
     */
    public function getCognitoTokensName(): string
    {
     ...
     * @return $this
     */
    public function setUser(AuthenticatableContract $user): AwsCognitoIdentityGuard
    {
        $this->user = $user;
     ...
     * @return array
     */
    protected function getDefaultAppConfig(): array
    {
     ...
     * @return $this
     */
    public function setRequest(Request $request): AwsCognitoIdentityGuard
    {
        $this->request = $request;
